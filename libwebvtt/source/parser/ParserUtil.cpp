...
namespace webvtt
{
  ...
  std::size_t ParserUtil::find_invalid(std::u8string_view s)
  {
                                             ...
                                             character)> &isLookedCharacter)
  {
    ...
    while (
        isLookedCharacter(*position) && position != input.end())
    {
                                   ...
                                   std::optional<uint32_t> additionalCharacter, bool isInAttribute, bool &parsingError)
  {
    try
    {
      ...
      case ParserUtil::HASHTAG_C:
        result = ParserUtil::parseHTMLNumberReference(input, position, parsingError);
        currentPosition = position;
        break;
      default:
        result = ParserUtil::parseHTMLNamedReference(input, position, isInAttribute, parsingError);
        currentPosition = position;
        break;
      }
      return result;
  ...
  bool ParserUtil::checkIfCSSSIdentifierBeginningRightFormat(std::u32string_view input)
  {
    if (input.empty())
      return false;
  ...
  bool ParserUtil::checkIfCSSIdentifierAllowedCharacter(char32_t one)
  {
